...
const InterviewCard = ({ interview, userType }) => {
    ...
    const canJoinSoon = moment(interview.scheduledDate).diff(moment(), 'minutes') <= 15 &&
        moment(interview.scheduledDate).diff(moment(), 'minutes') >= -30;

    return (
        <div className={`border rounded-lg p-4 hover:shadow-md transition-all cursor-pointer ${isToday ? 'border-blue-300 bg-blue-50' : 'border-gray-200 hover:border-gray-300'
            }`}>
            {/* Header */}
            <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                    <h4 className="font-semibold text-gray-900 text-sm">
                        {userType === 'candidate'
                            ? `Interview with ${interview.company?.name || 'Company'}`
                            : `Interview: ${interview.candidate?.name || 'Candidate'}`
                        }
                    </h4>
                    <div className="flex items-center gap-2 mt-1">
                        {userType === 'candidate' ? (
                            <div className="flex items-center gap-1 text-xs text-gray-600">
                                ...
                                {interview.jobPosition || 'Position'}
                            </div>
                        ) : (
                            <div className="flex items-center gap-1 text-xs text-gray-600">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    {canJoinSoon && interview.zoomJoinUrl && (
                        <button
...
const UpcomingInterviews = ({ userType = 'candidate', limit = 5 }) => {
        ...
        const fetchUpcomingInterviews = async () => {
            try {
                ...
                const endpoint = userType === 'candidate'
                    ...
                    : `${backendUrl}/interview/recruiter`;
                ...
                if (data.success) {
                    ...
                    const upcoming = data.interviews
                        .filter(interview => {
                            return moment(interview.scheduledDate).isAfter(moment()) &&
                                interview.status === 'scheduled';
                        })
                        .sort((a, b) => moment(a.scheduledDate).diff(moment(b.scheduledDate)))
                        .slice(0, limit);
    ...
    }, [token, limit, userType, backendUrl]);
